...
function widthForHeader(header: string) { if (header === "만족도_6번_주관식_소감") return 44; if (header.includes("시각")) return 26; if (["성명", "학번", "학과"].includes(header)) return 18; if (header.startsWith("CES-D_") || header.startsWith("PSS-10_") || header.startsWith("만족도_응답_")) return 15; return Math.max(14, Math.min(28, header.length + 5)); }
...
export function buildRawAssessmentXlsx(rows: RawAssessmentExportRecord[]) {
  ...
  const headers = ["응답_ID", "제출_시각_UTC", "언어", "성명", "학번", "학과", "만족도_6번_주관식_소감", "개인정보_수집이용_동의", "개인정보_동의시각_UTC", "상담센터_연락_동의", "상담센터_연락_동의시각_UTC", "CES-D_총점", "PSS-10_총점", "만족도_총점", "만족도_응답수", ...Array.from({ length: 20 }, (_, index) => `CES-D_${index + 1}`), ...Array.from({ length: 10 }, (_, index) => `PSS-10_${index + 1}`), ...questionIds.map(questionId => `만족도_응답_${questionId}`)];
  ...
  const commentsWorksheet = XLSX.utils.aoa_to_sheet([["응답_ID", "제출_시각_UTC", "성명", "학번", "학과", "만족도_6번_주관식_소감"], ...parsedRows.filter(row => Boolean(row.satisfactionComment?.trim())).map(row => [row.id, formatTimestamp(row.createdAt), protectText(row.studentName), row.studentId, protectText(row.department), protectText(row.satisfactionComment)])]);
  commentsWorksheet["!cols"] = [{ wch: 12 }, { wch: 26 }, { wch: 18 }, { wch: 18 }, { wch: 18 }, { wch: 50 }];
