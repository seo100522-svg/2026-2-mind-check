...
describe("buildRawAssessmentXlsx", () => {
  it("creates an Excel binary with Korean text, a text student ID, scores, and answer columns", () => {
    const bytes = buildRawAssessmentXlsx([{
      id: 1, studentName: "홍길동", studentId: "020261234", contactNumber: "010-1234-5678", department: "성장학과", locale: "ko", cesdScore: 16, pssScore: 12, satisfactionScore: 5, satisfactionAnswerCount: 1, satisfactionComment: "한글 의견",
    ...
    expect(worksheet["E1"]?.v).toBe("연락처");
    expect(worksheet["E2"]?.v).toBe("010-1234-5678");
    expect(worksheet["F2"]?.v).toBe("020261234");
    expect(worksheet["G2"]?.v).toBe("성장학과");
    expect(worksheet["H1"]?.v).toBe("만족도_6번_주관식_소감");
    expect(worksheet["H2"]?.v).toBe("한글 의견");
    expect(worksheet["M2"]?.v).toBe(16);
    expect(worksheet["Q2"]?.v).toBe(1);
    expect(worksheet["AK2"]?.v).toBe(2);
    expect(worksheet["AU2"]?.v).toBe(5);
    expect(workbook.SheetNames[0]).toBe("주관식 소감");
    expect(workbook.Sheets["주관식 소감"]?.["D2"]?.v).toBe("010-1234-5678");
    expect(workbook.Sheets["주관식 소감"]?.["G2"]?.v).toBe("한글 의견");
  });
});
