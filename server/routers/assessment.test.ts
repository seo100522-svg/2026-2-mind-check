...
describe("healing play assessment router", () => {
  ...
  it("scores a consented response and records rated and free-text satisfaction answers", async () => {
    ...
    const result = await assessmentRouter.createCaller({} as TrpcContext).submit({ anonymousSessionId: "e995be3f-8e37-4580-af8d-85fdc84c89eb", locale: "ko", studentName: "홍길동", studentId: "20261234", contactNumber: "010-1234-5678", department: "성장학과", personalDataConsent: true, counselingContactConsent: true, cesdAnswers: Array(20).fill(0), pssAnswers: Array(10).fill(0), satisfactionAnswers });
    expect(result).toMatchObject({ cesdScore: 12, pssScore: 20, satisfactionAnswerCount: 5 });
    expect(mocks.saveAssessmentResponse).toHaveBeenCalledWith(expect.objectContaining({ contactNumber: "010-1234-5678", department: "성장학과", satisfactionAnswers }));
  });
  it("rejects submission without the required personal-data consent", async () => {
    await expect(assessmentRouter.createCaller({} as TrpcContext).submit({ anonymousSessionId: "e995be3f-8e37-4580-af8d-85fdc84c89eb", locale: "ko", studentName: "홍길동", studentId: "20261234", contactNumber: "010-1234-5678", department: "성장학과", personalDataConsent: false, counselingContactConsent: false, cesdAnswers: Array(20).fill(0), pssAnswers: Array(10).fill(0), satisfactionAnswers: [] } as never)).rejects.toThrow();
    expect(mocks.saveAssessmentResponse).not.toHaveBeenCalled();
  });
  it("rejects a malformed prize-notification contact number", async () => {
    await expect(assessmentRouter.createCaller({} as TrpcContext).submit({ anonymousSessionId: "e995be3f-8e37-4580-af8d-85fdc84c89eb", locale: "ko", studentName: "홍길동", studentId: "20261234", contactNumber: "not-a-number", department: "성장학과", personalDataConsent: true, counselingContactConsent: false, cesdAnswers: Array(20).fill(0), pssAnswers: Array(10).fill(0), satisfactionAnswers: [] } as never)).rejects.toThrow("연락처 형식을 확인해 주세요.");
    expect(mocks.saveAssessmentResponse).not.toHaveBeenCalled();
  });
